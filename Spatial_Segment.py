...
def clear_output_folder(folder_path="KMeans_Clouds"):
    ...
    # Create subfolders for RGB and PointStack_Ready
    os.makedirs(os.path.join(folder_path, "RGB"))
    os.makedirs(os.path.join(folder_path, "PointStack_Ready"))
...
if __name__ == "__main__":
    ...
    if file_path:
        ...
        # Clear output folder
        clear_output_folder("KMeans_Clouds")
        ...
        for i, (cluster_points, cluster_colors, cluster_normals, cluster_labels, small_pcd) in enumerate(segmented_clouds):
            output_txt_rgb = f"KMeans_Clouds/RGB/{base_name}_{i}_rgb.txt"
            output_txt_nocolor = f"KMeans_Clouds/PointStack_Ready/{base_name}_{i}.txt"
        ...
        # Visualize all smaller point clouds together
